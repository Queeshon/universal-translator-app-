...
class ConversationsController < ApplicationController
  ...
  def create
    ...
    if @conversation.valid? && check_uniqueness
      @conversation.save
      redirect_to conversation_path(@conversation)
    else
      flash[:errors] = @conversation.errors.full_messages
      if !check_uniqueness
        flash[:errors] << "You are already linked."
      end
...
private
  ...
  def check_uniqueness
    !Conversation.any?{|convo| convo.users.values == [@conversation.sender, @conversation.recipient] || convo.users.values == [@conversation.recipient, @conversation.sender]}
  end
